...
// cheking wheather given num is above 0 to below 500
  ...
  if(inpnum){
    if (inpnum >= 0 && inpnum <= 500) {
      //  if condition true then call fetch-fact function
        fetch_fact(inpnum);
        
    }
    // if not true then execute else block with message text
    else{
        fact_container.style.display = "block";
  ...
  // if user did not give any input value then it will execute
  else{
    fact_container.style.display = "block";
